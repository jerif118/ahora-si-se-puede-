...
from python_code.watermark import watermark, get_latest_f
...
def start_request():
    ...
    app.logger.info('Inicio de la solicitud: %s %s %s %s', 
        ...
        if request.method == 'POST':
            ...
            #     print(statusFunction1, msgF1)
                ...
                if statusFunction and statusFunction1:
                    # response_data['message'] = 'Embeddings guardados, imagen recortada, imagen redimensionada correctamente'
                    ...
                    response_data['message'] = 'Hubo un error procesar la imagen'
            except Exception as e:
              print(str(e))
              response_data['statusUser'] = '3'
              return jsonify(response_data), 500
...
def login():
    ...
    data_db = get_user_DB(str(username))
    if data_db == None:
        default_response['message'] = "Hay un error al conectar a la base de datos"
        return default_response, 500
    pass_in_db = data_db[0][1]
    # print(data[0][1])
    try:
...
CLAVE_SECRETA_ESPECIFICA = 'clave-secreta-especifica'
def verificarToken_est(f):
    @wraps(f)
    def decorador(*args, **kwargs):
        token = request.headers.get('mi-token-especifico')
        if not token:
...
def validacion_estd():
    ...
    try:
        ...
        path = "./fotitos"
        latest_folder = get_latest_f(path)
        ...
        url_photo= watermark(user,path_dni)
...
def validacion_estd1():
    ...
    try:
        ...
        path_file = os.path.join(path, str(dni), "recortada")
